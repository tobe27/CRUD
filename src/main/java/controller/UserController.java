    ...
    public String add(@Validated Supplier supplier) throws Exception {
        ...
        //异常处理：当添加信息为空时，异常错误跳转到error界面
            if (supplier.getSupplierName().isEmpty()||supplier.getContactName().isEmpty()
                ...
                return "redirect:/supplierPage";  //重定向到列表页面
            }
    }
...
//          异常处理，当供应商不存在时，跳转到error页面，并返回错误信息
    ...
    public ModelAndView updatePage(@PathVariable("supplierId") String supplierId) throws Exception {
        ModelAndView modelAndView=new ModelAndView();
        Supplier supplier=supplierService.getSupplierById(supplierId);
    ...
    public String update(@PathVariable("supplierId") String supplierId,Supplier supplier) throws Exception {
